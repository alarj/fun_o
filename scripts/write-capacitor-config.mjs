...
const buildMode = String(process.argv[3] || process.env.FUNO_COMPETITOR_APP_BUILD || "debug").trim().toLowerCase();
...
if (!["debug", "release"].includes(buildMode)) {
  throw new Error(`Unknown build mode "${buildMode}". Use "debug" or "release".`);
}
...
process.stdout.write(`Wrote ${path.basename(configPath)} for ${mode} / ${buildMode} mode.\n`);
